...
var TrainOrder = React.createClass({
	getInitialState:function(){
		return{
			...
			tel:'',
			userName:'',
			// 这个字段用来控制按钮状态（有时保险加载慢不能给提交）
			buttonState:false
		}
	},
	// 保险信息获取后改变按钮状态
	buttonStateFn:function(){
		this.setState({
			buttonState:true
		})
	},
	...
	handleSubmit: function(e){
		...
		if(this.state.banquetName == "硬座"){
				banquet="1"
		}else if(this.state.banquetName == "软座"){
				banquet="2"
		}else if(this.state.banquetName == "硬卧"){
				banquet="3"
		}else if(this.state.banquetName == "软卧"){
				banquet="4"
		}else if(this.state.banquetName == "高级软卧"){
				banquet="6"
		}else if(this.state.banquetName == "二等座"){
				banquet="O"
		}else if(this.state.banquetName == "一等座"){
				banquet="M"
		}else if(this.state.banquetName == "特等座"){
				banquet="P"
		}else if(this.state.banquetName == "商务座"){
				banquet="9"
		}
		// 所有条件都通过以后做数据存储
		this.localStorageFn();
		...
		var form={
			...
			"expressAmount": this.state.kuaidi? 7+(this.state.userInfo.length*5) : '',
			...
			"loginUserName": this.state.trainUser=="1"?this.state.loginUserName:"",
			"loginUserPassword": this.state.trainUser=="1"?this.state.loginUserPassword:"",
	...
	render:function(){
		...
		return(
				<div className="order">
					<form onSubmit={this.handleSubmit} autocomplete="off">
						...
						{this.addressTianxie()}
						<Insurance baoxian={this.state.baoxian} baoXianFn={this.baoXianFn} getInsurancePrice={this.getInsurancePrice} buttonStateFn={this.buttonStateFn}/>
     ...
     开车前48小时～15天期间内，改签或变更到站至距开车15天以上的其他列车，又在距开车15天前退票的，仍核收5%的退票费。</p></div>
						{this.state.buttonState?<div className="sub"><input type="submit" value="提交订单" className="submit"/></div>:<div className="sub"><input type="button" value="提交订单" className="submit"/></div>}
	...
	render:function(){
		...
		return(
				<div className="ticket_type">
					...
					<div className={(this.state.tips)?"tips cur":"tips"}>卧铺显示为下铺票价、支付时暂收下铺价格,差价1-2个工作日原路返回。</div>
				</div>
			)
	}
});
...
var Linkman =React.createClass({
	...
	userFn:function(){
		...
		uddh5.bridge(native_callback,requestHybrid);
		// that.props.userInfoFn([        {
  //           "commonContacts": 1, //是否添加常用联系人
  //           "passengerName": "李**", //乘客
  //           "passengerCertificateType": "1", //身份证类型
  //           "passengerCertificateTypeName": "二代身份证", //类型名称
  //           "passengerCertificateNo": "14262319940526****",//证件号
  //           "passengerBirthday": "1994-05-26", //出生日期(yyyy-MM-dd)
  //           "passengerType": 1, //票类型
  //           "passengerTypeName": "成人票",//票名称
  //           "ticketAmount": 6 //车票金额
  //       }]);
	},
	// 通讯录
...
{/*var Ckecked = React.createClass({
	...
	render:function(){
		return(
				<div className="checked_contKuaidi">
					<div className="contKuaidi">
...
var Insurance = React.createClass({
	...
	render:function(){
		return(
				<div className="checked_contKuaidi checked_cont insurance">
					<div className="contKuaidi">
						<div className="top">
