...
var HotelIntro=React.createClass({
  ...
  componentDidMount: function(){
    ...
    $.post(uddh5.apihost+'/gethotelfacinfo', {id: hotelid}, function(data){
      var data=data.data;
      if(this.isMounted()){
        this.setState({
    ...
    return (
      <div className="hotel-intro">
        ...
        <div className="hi-content">
          ...
          <Link className={'intro item'} to={{ pathname: "/info/"+uddh5.location.queryHash().split("/")[2], query: {starttime: this.props.starttime, endtime: this.props.endtime}}}>
...
var RoomList=React.createClass({
  ...
  ToggleClick: function(elem, event){
          ...
          return (
            <div id={'roomlist_'+index} key={index} className={(index == 0)? 'room-item open' : 'room-item'}>
                <div className="room-info clearfix">
                      <Link className="media pull-left" to={{pathname: "/roominfo", query: {productId: elem.hotelProduct[0].productId, starttime: this.props.starttime, endtime: this.props.endtime, productType: elem.hotelProduct[0].producttype, elonghotelId: elem.hotelProduct[0].elonghotelId || '0', elonghotelCode: elem.hotelProduct[0].elonghotelCode || '0', elongroomTypeId: elem.hotelProduct[0].elongroomtypeId || '0', elongproductId: elem.hotelProduct[0].elongproductId || '0'}}}>
                      ...
                      <div className="room-book absolute-middle" onClick={this.ToggleClick.bind(this, elem)}>
                          <div className="price">
                    ...
                    item.productName? <div key={num} className="sub-room padding-inner">
                        <div className="sub-room-inner clearfix">
                          <div className="media pull-left">
                            <h3><Link to={{pathname: "/room",query: {productId: item.productId, starttime: this.props.starttime, endtime: this.props.endtime, productType: item.producttype, elonghotelId: item.elonghotelId || '0', elonghotelCode:item.elonghotelCode || '0', elongroomTypeId: item.elongroomtypeId || '0', elongproductId:item.elongproductId || '0'}}}>{item.productName}</Link></h3>
                          ...
                          <div className="room-book clearfix absolute-middle">
                            ...
                            {item.issell ==1? <div className="order-btn yellow-btn">{this.props.isLogin? <Link to={{ pathname: "/order/"+uddh5.location.queryHash().split("/")[2]+"/"+elem.roomTypeId+"/"+item.productId, query: {starttime:this.props.starttime, endtime: this.props.endtime, productType: item.producttype, elonghotelId: item.elonghotelId || '0', elonghotelCode:item.elonghotelCode || '0', elongroomTypeId: item.elongroomtypeId || '0', elongproductId:item.elongproductId || '0'}}} onClick={item.paymenttype == 2 || item.paymenttype == 3? this.props.updateVersionRemind : ''}>
                                <p>预订</p>
    ...
    return (
      <div>
          ...
          <div style={{position: 'relative'}}>
            ...
            <RoomList isLogin={this.props.isLogin} roomType={this.state.roomType} starttime={this.props.starttime} endtime={this.props.endtime} hotelname={this.props.hotelname} getLogin={this.props.getLogin} updateVersionRemind={this.props.updateVersionRemind}/>
          </div>
      </div>
    );
  }
});
var RemindUpdate=React.createClass({
  render: function(){
    return(
      <div className="box_tip">
        <div className="alertQx">
          <h3>预定到店付酒店请升级到最新APP！</h3>
          <div className="line"></div>
          <div className="btn">
            <a className="btn-full" href="javascript:void(0);" onClick={this.props.closeRemind}>知道了</a>
          </div>
        </div>
      </div>
    )
...
var HotelDetail=React.createClass({
  ...
  componentDidMount: function(){
      ...
      $.post(uddh5.apihost+'/gethotelbasisinfo', {id:uddh5.location.queryHash().split("/")[2], userId: uddh5.getCookie('userId')}, function(data){
        var data=data.data;
        this.setState({
  ...
  updateVersionRemind: function(event){
  ...
  render: function(){
    ...
    return (
      <div className="hotel-info-inner">
        ...
        <RoomFilter isLogin={this.state.isLogin} getLogin={this.getLogin} getTime={this.getTime} starttime={this.state.starttime} endtime={this.state.endtime} updateVersionRemind={this.updateVersionRemind}/>
        {this.state.remindShow? <RemindUpdate closeRemind={this.closeRemind} /> : ''}
      </div>
    );
  }
});
